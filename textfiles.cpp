...
void parseCSV()
{
	ifstream data("Book1.csv");               //abrir y leer los archivos del book1.csv
	string line;                              //Crear una variable que almacena datos
	...
	while(getline(data,line))                 //ciclo while que realiza la funcion del archivo data("Book1.csv") y ingresa esos datos de la avariable anterior
    {
		stringstream lineStream(line);
		string cell;
		vector<string> parsedRow;
		colcount=0;                           //constante de colcount=0
		while(getline(lineStream,cell,','))   //ciclo while que realiza la funcion de revisar cada renglon e idintificar cada ',' y guarda esa informacion
	    {
